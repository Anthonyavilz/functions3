...
let divide2 = numbers.map((num) => num / 2)
console.log(divide2)
...
let endsWith = cities.filter(element => element.endsWith('i'))
...
/* 
    Using forEach method, identify each element of the 'fruits' array above if it's an apple or orange by its color. If the color is red, print 
    "The fruit with index [THE ELEMENT'S INDEX] is an apple." 
...
fruits.forEach((element) => {
  if(element.color === 'red'){
    ...
    console.log(`The fruit with index ${fruits.indexOf(element)} is an orange.`)
  } else {
    console.log(`The fruit with index ${fruits.indexOf(element)} is neither apple or orange.`)
  }
})
...
let total = foods.filter((element) => element.tags.includes('rice')).reduce((acc, curr) => acc + curr.price, 0)
console.log(total)
